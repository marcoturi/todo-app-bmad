...
2. **Given** the frontend codebase is complete, **When** the security review covers the React app, **Then** the review confirms: no `dangerouslySetInnerHTML` usage, no secrets or API keys committed in source, `VITE_API_URL` is the only env var exposed to the client bundle, and all user-supplied content is rendered as text (not HTML). **And** findings are documented in `docs/security-review.md`.
...
- [ ] Task 1: Backend Security Audit (AC: #1)
  - [ ] 1.1 Verify all SQL queries use postgres.js tagged template literals (parameterised) — no raw string concatenation
  - [ ] 1.2 Confirm all API responses are JSON (no raw HTML rendering, no XSS vectors)
  - [ ] 1.3 Verify TypeBox schemas cover ALL request bodies and path params across all routes
  - [ ] 1.4 Confirm @fastify/helmet is registered globally with appropriate config
  - [ ] 1.5 Check CORS config restricts to `FRONTEND_URL` only (not wildcard)
  - [ ] 1.6 Verify no hardcoded secrets or credentials in source files
  - [ ] 1.7 Check environment variable validation (TypeBox schema for env vars)
  - [ ] 1.8 Verify Docker images run as non-root users
  - [ ] 1.9 Document all findings in `docs/security-review.md`

- [ ] Task 2: Frontend Security Audit (AC: #2)
  - [ ] 2.1 Confirm zero `dangerouslySetInnerHTML` usage
  - [ ] 2.2 Verify no secrets/API keys committed in source (check .gitignore coverage)
  - [ ] 2.3 Confirm only `VITE_API_URL` is exposed to client bundle (check `VITE_SENTRY_DSN` is acceptable)
  - [ ] 2.4 Verify all user-supplied content renders as text, not HTML
  - [ ] 2.5 Check dependency vulnerabilities via `pnpm audit`
  - [ ] 2.6 Document findings in `docs/security-review.md`

### Performance Baseline

- [ ] Task 3: Build Analysis (AC: #5)
  - [ ] 3.1 Run `pnpm --filter apps/web build` and capture output
  - [ ] 3.2 Verify code splitting produces multiple chunks (vendor-react, vendor-redux, vendor-radix, vendor-sentry, vendor-ui)
  - [ ] 3.3 Verify no individual JS chunk exceeds 500kb uncompressed
  - [ ] 3.4 Document bundle sizes and chunk breakdown in `docs/performance-baseline.md`

- [ ] Task 4: Runtime Performance Measurement (AC: #3, #4)
  - [ ] 4.1 Start full stack via `docker compose up --build`
  - [ ] 4.2 Measure page load to interactive — target: < 2s
  - [ ] 4.3 Measure create-todo round-trip (form submit → list update) — target: < 200ms
  - [ ] 4.4 Measure `GET /api/v1/todos` API response time — target: < 300ms p95
  - [ ] 4.5 If any target fails, document investigation path and remediate critical path
  - [ ] 4.6 Document all results with pass/fail in `docs/performance-baseline.md`

### Documentation

- [ ] Task 5: Create Security Review Document (AC: #1, #2)
  - [ ] 5.1 Write `docs/security-review.md` with findings table (severity: critical/high/medium/low)
  - [ ] 5.2 Ensure zero critical or high findings remain unaddressed

- [ ] Task 6: Create Performance Baseline Document (AC: #3, #4, #5)
  - [ ] 6.1 Write `docs/performance-baseline.md` with NFR targets table and results
  - [ ] 6.2 Include build output / bundle analysis evidence
  - [ ] 6.3 Include runtime measurement results with methodology
